...
@section('title', (app()->getLocale() === 'bn' && $event->title_bn ? $event->title_bn : $event->title) . ' | CPRC Events')
    ...
    <div class="container-col-8">
      <article style="background:#fff; border-radius:var(--radius-medium); padding:var(--spacing-large); box-shadow:var(--shadow-small);">
        <a href="{{ route('events.index') }}" style="color:var(--color-link-normal); text-decoration:none; font-size:var(--text-small); display:inline-block; margin-bottom:var(--spacing-medium);">
          <i class="ph ph-arrow-left"></i> {{ app()->getLocale() === 'bn' ? 'অনুষ্ঠানসমূহে ফিরে যান' : 'Back to Events' }}
        </a>
        @if($event->image)
        ...
        @endif
        <h1 style="color:var(--color-primary-bg); margin:0 0 var(--spacing-medium);">{{ app()->getLocale() === 'bn' && $event->title_bn ? $event->title_bn : $event->title }}</h1>
        <div style="display:flex; gap:16px; margin-bottom:var(--spacing-large); flex-wrap:wrap; font-size:var(--text-small); color:#555;">
          ...
          @endif
          @if($event->venue)
            <span><i class="ph ph-map-pin"></i> {{ app()->getLocale() === 'bn' && $event->venue_bn ? $event->venue_bn : $event->venue }}</span>
          @endif
        </div>
        <div style="line-height:1.8; font-size:var(--typography-body-font-size);">
          {!! app()->getLocale() === 'bn' && $event->description_bn ? $event->description_bn : $event->description !!}
        </div>
      </article>
    </div>
    <div class="container-col-4">
      <section class="widget notice-news-card-widget">
        <div class="notice-card">
          <p class="notice-title"><i class="ph ph-calendar-star"></i> {{ app()->getLocale() === 'bn' ? 'অন্যান্য অনুষ্ঠান' : 'Other Events' }}</p>
              ...
              <li class="notice-content-list">
                <a class="notice-link" href="{{ route('events.show', $other->slug) }}">
                  ...
                  <div class="notice-text-wrap">
                    <p class="notice-text">{{ app()->getLocale() === 'bn' && $other->title_bn ? $other->title_bn : $other->title }}</p>
