  ...
  <div class="cprc-pkg-grid cprc-pkg-grid-full">
    @foreach($packages as $pkg)
      <div class="cprc-pkg-card {{ $pkg->is_featured ? 'cprc-pkg-featured' : '' }}">
        ...
        <h3 class="cprc-pkg-name">{{ app()->getLocale() === 'bn' ? $pkg->name_bn : $pkg->name }}</h3>
        ...
        <a href="{{ route('contact') }}?package={{ $pkg->slug }}" class="cprc-pkg-btn {{ $pkg->is_featured ? 'cprc-pkg-btn-featured' : '' }}">
          <i class="ph ph-calendar-check"></i> {{ __('site.enquire_now') }}
        </a>
      </div>
    @endforeach
  </div>
